...
/*
        ...
        void OnEnable()
        {
            ...
            shaderDisableBatching = typeof(Shader).GetProperty("disableBatching",
                BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.GetProperty);

            //disableBatchingType = Type.GetType( "UnityEngine.DisableBatchingType, UnityEngine" );

            editorGUIMouseButtonDown = typeof(EditorGUI).GetMethod("ButtonMouseDown", privStatic, null,
                new Type[] {typeof(Rect), typeof(GUIContent), typeof(FocusType), typeof(GUIStyle)}, null);
            ...
            guiLayoutUtilityTopLevel = typeof(GUILayoutUtility).GetProperty("topLevel",
        ...
        void InitStyles()
        {
            Styles.errorIcon = (Texture2D) editorGuiUtilityLoadIcon.Invoke(null, new object[] {"console.erroricon.sml"});
                // EditorGUIUtility.LoadIcon( "console.erroricon.sml" );
            Styles.warningIcon =
                (Texture2D) editorGuiUtilityLoadIcon.Invoke(null, new object[] {"console.warnicon.sml"});
                // EditorGUIUtility.LoadIcon( "console.warnicon.sml" );
            Styles.showSurface =
                (GUIContent)
                editorGuiUtilityTextContent.Invoke(null,
                    new object[] {"Show generated code|Show generated code of a surface shader"});
                // EditorGUIUtility.TextContent( "Show generated code|Show generated code of a surface shader" );
            Styles.showCurrent =
                (GUIContent) editorGuiUtilityTextContent.Invoke(null, new object[] {"Compile and show code | ▾"});
                // new GUIContent( "Compile and show code | ▾" );
            Styles.messageStyle = "CN StatusInfo";
            Styles.evenBackground = "CN EntryBackEven";
            Styles.no = (GUIContent) editorGuiUtilityTextContent.Invoke(null, new object[] {"no"});
                // EditorGUIUtility.TextContent( "no" );
            Styles.builtinSurfaceShader =
                (GUIContent) editorGuiUtilityTextContent.Invoke(null, new object[] {"Built-in surface shader"});
                // EditorGUIUtility.TextContent( "Built-in surface shader" );
        ...
        public override void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                if (
                    GUILayout.Button(
                        new GUIContent("Replace with Shader Forge shader", SF_Styles.IconWarningSmall,
                            "This will erase any existing shader code"), hasShaderForgeData ? "Button" : "MiniButton"))
                {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    if (GUILayout.Button("Open shader code", btnStyle))
                    {
                        UnityEditorInternal.InternalEditorUtility.OpenFileAtLineExternal(
                            AssetDatabase.GetAssetPath(shader), 1);
                    }
        ...
        public void DrawUnitysInspector()
        {
            ...
            if (shader.isSupported)
            {
                EditorGUILayout.LabelField("Cast shadows",
                    (!(bool) sutilHasShadowCasterPass.Invoke(null, new object[] {shader})) ? "no" : "yes",
                    new GUILayoutOption[0]);
                EditorGUILayout.LabelField("Render queue",
                    ((int) sutilGetRenderQueue.Invoke(null, new object[] {shader})).ToString(
                        CultureInfo.InvariantCulture), new GUILayoutOption[0]);
                EditorGUILayout.LabelField("LOD",
                    ((int) sutilGetLOD.Invoke(null, new object[] {shader})).ToString(CultureInfo.InvariantCulture),
                    new GUILayoutOption[0]);
                EditorGUILayout.LabelField("Ignore projector",
                    (!(bool) sutilDoesIgnoreProjector.Invoke(null, new object[] {shader})) ? "no" : "yes",
                    new GUILayoutOption[0]);
                string label;
